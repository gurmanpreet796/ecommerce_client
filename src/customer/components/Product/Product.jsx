...
import { FormControlLabel ,FormControl, Radio,FormLabel,RadioGroup} from '@mui/material'
...
export default function Product() {
  ...
  const handleFilter=(value, sectionId)=>{
    ...
    const searchParmamms=new URLSearchParams(location.search)

    let filterValue=searchParmamms.getAll(sectionId)

    if(filterValue.length>0 && filterValue[0].split(",").includes(value)){
      filterValue=filterValue[0].split(",").filter((item)=>item!=value);

      if(filterValue.length===0){
        searchParmamms.delete(sectionId)
      }
    ...
    if(filterValue.length>0){
      searchParmamms.set(sectionId, filterValue.join(","));
    
    }
    const query=searchParmamms.toString();
    navigate({search:`?${query}`})
  }
  const handleRadioFilterChange=(e, sectionId)=>{
    const searchParmamms= new URLSearchParams(location.search)
    searchParmamms.set(sectionId,e.targetvalue)
    const query=searchParmamms.toString();
    navigate({search:`?${query}`})
  }
  return (
